...
namespace YG
{
    public partial class YandexGame
    {
        ...
        {
            savesData.Integers[id] = value;
            SaveProgress();
        }

        public static int GetInt(string id, int defaultValue = 0)
        {
            return savesData.Integers.GetValueOrDefault(id, defaultValue);
        }

        public static void SetFloat(string id, float value)
        {
            savesData.Floats[id] = value;
            SaveProgress();
        }

        public static float GetFloat(string id, int defaultValue = 0)
        {
            return savesData.Floats.GetValueOrDefault(id, defaultValue);
        }

        public static void SetString(string id, string value)
        {
            savesData.Strings[id] = value;
            SaveProgress();
        }

        public static string GetString(string id, string defaultValue = "")
        {
            return savesData.Strings.GetValueOrDefault(id, defaultValue);
        }

        public static bool HasObject(string id)
        {
            return savesData.Integers.ContainsKey(id) || savesData.Strings.ContainsKey(id) ||
                   savesData.Floats.ContainsKey(id);
        }
        ...
        public static void DeleteObject(string id)
        {
            ...
            {
                savesData.Integers.ToDictionary(k => k.Key, v => (object)v.Value),
                savesData.Strings.ToDictionary(k => k.Key, v => (object)v.Value),
                savesData.Floats.ToDictionary(k => k.Key, v => (object)v.Value),
            };
            
            foreach (var dict in dictionaries.Where(dict => dict.ContainsKey(id)))
            {
                dict.Remove(id);
            }
        }
