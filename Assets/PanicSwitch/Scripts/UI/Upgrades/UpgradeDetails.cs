...
public class UpgradeDetails : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Level = YandexGame.GetInt("Level" + Id);

        if (YandexGame.GetInt("_isprocess" + Id) > 0)
        {
            timer = YandexGame.GetFloat("timer" + Id);

            if (TimeLastSession.ts.TotalSeconds >= TimeResearch[Level])
            {
                UpdateLevel();
                _isprocess = 0;
                YandexGame.SetInt("_isprocess" + Id, 0);
            }
            else
            {
                timer -= (float)(TimeLastSession.ts.TotalSeconds + YandexGame.GetFloat("SessionTimer"));
                tempTimer = TimeResearch[Level] - timer;
            }
        }

        _isprocess = YandexGame.GetInt("_isprocess" + Id);
    ...
    private void Update()
    {
        if (_isprocess == 1)
        {
            if (timer > 0)
            {
                ...
                timerDisplayText.text = string.Format("{0:0}:{1:00}:{2:00}", (int)(timer / 3600),
                    (int)(timer / 60 - (int)(timer / 3600) * 60),
                    (int)((timer - (int)(timer / 60) * 60))); // мин. и секунды
            }
            else
            {
                _isprocess = 0;
    ...
    {
        timer = Level >= TimeResearch.Length ? TimeResearch[^1] : TimeResearch[Level];
        _isprocess = 1;
        CheckProcess(_isprocess);
        YandexGame.SetInt("_isprocess" + Id, 1);
        SaveTimer();
        _clickAudios.Play();
    }
    ...
    private void UpdateText()
    {
        ...
        var text = YandexGame.lang == "ru" ? "МАКС." : "MAX";
        priceDisplayText.text = Level >= Price.Length ? text : Price[Level].ToString();
        gemPriceDisplayText.text =  Level >= gemPrice.Length ? text : gemPrice[Level].ToString();
    }

    private void UpdateLevel()
    {
        Level++;
        YandexGame.SetInt("Level" + Id, Level);
    ...
    {
        YandexGame.SetFloat("timer" + Id, timer);
    }
}